...
SEO:
- Local SEO → ₹9,600/month (Google Business Profile, 'near me' rankings, citations)
- On-Page + Off-Page SEO → ₹14,400/month (complete ranking package)
- GEO (Generative Engine Optimization) → ₹18,000/month (rank in Google AI Overviews, ChatGPT, Perplexity — new in 2026)
- E-Commerce SEO → ₹23,000/month (Amazon, Flipkart, Shopify)
- Technical SEO Audit → ₹12,000 one-time
- Keyword Research & Strategy → ₹6,000 one-time
...
→ ROI framing: "₹10,000 invest karoge, ₹40,000-50,000 return milta hai — yahi humara 4.5x ROI hai"
→ Smaller entry point suggest karo (e.g. Local SEO ₹9,600 se start)
...
export function buildPrompt(userMessage, conversationHistory, stage, stageNotes) {
    ...
    // Add the current user message if it's not already in history (though usually it is added after AI reply)
    // But for prompt building, we just need the system prompt and history.
    // The callGemini will use this systemPrompt and history.
...
export function getMediaReply(messageType) {
    switch (messageType) {
        ...
        case 'audio':
        case 'ptt':
            return "Voice note mila! 🎙️ Abhi text mein reply nahi kar sakta audio ke liye — please type mein batao ya seedha call karein: adsverse.in/contact";
        case 'video':
...
// Keeping legacy exports for compatibility if needed elsewhere, though Task 2 calls for FULL REWRITE
...
export function isEscalationRequest(message) {
    // This is now handled by isHumanTrigger, but keeping for compatibility
    return isHumanTrigger(message).triggered;
}

export function getIntentTags(userMessage) {
    // Legacy support for leadManager
    return [];
}
